...
class Screens():
    ...
    def config(self, surface, windowx, windowy, lang):
        ...
        with open("txt/"+lang+"/expl.txt") as expl:
            self.explarr = expl.readlines()   # explanations
        with open("txt/"+lang+"/gui_expl.txt") as gui_expl:
            self.gui_explarr = gui_expl.readlines()   # gui explanations
        with open("txt/"+lang+"/rods_expl.txt") as rods_expl:
            self.rods_explarr = rods_expl.readlines()   # rods explanations
        with open("txt/"+lang+"/texts.txt") as texts:
            self.texts_arr = texts.readlines()   # texts
        self.graph_index = 0
        
    # main menu, without main screen
    def main(self, mouse, pause, sound, record):
        if pause is True:
            pygame.draw.rect(self.surface, rgb.green1, (177, 1, 30, 30))   # if simulation is paused: color it
        if record is True:
            pygame.draw.rect(self.surface, rgb.red1, (177, 32, 30, 30))   # if recording: color it
        ...
        if pause is True:
            self.surface.blit(self.runbt, (177, 1))   # if simulation is paused: show image
        else:
            self.surface.blit(self.stopbt, (177, 1))
        if sound is True:
            self.surface.blit(self.soundon, (208, 1))   # if sound is on
        else:
            self.surface.blit(self.soundoff, (208, 1))
    ...
    def settings(self, mouse, antial, sound_volume, zoom, lang):
        ...
        if antial is True:
            pygame.draw.rect(self.surface, rgb.green1, (177, 1, 30, 30))
    ...
    def picker(self, mouse, vals, val_names, val_colors, will_rec, will_graph):
        ...
        for word_repeat in range(2):   # repeat it twice:
            if word_repeat == 1:
                vertical_lines = vertical_lines[5:]   # in second iterration move to right side
        ...
        for num, val_name in enumerate(val_names):   # for every value:
            ...
            if will_rec[num] is True:   # if this value will be recorded:
            ...
            if will_graph[num] is True:   # if this value will be graphed:
    ...
    def rec_show(self, mouse, mouse_wheel, records, sel_record, read_record, header_record, color_record, antial):
        pygame.draw.rect(self.surface, (255, 255, 55), (1, 1, self.windowx-2, self.windowy-2))   # blank screen
        gr.mouseover_coord(self.surface, 1, 1, rgb.gray1, mouse)   # back button mouseover effect
        ...
        if self.graph_index < 0:
            self.graph_index = 0   # prevent index from going negative
        if self.graph_index > len(read_record) - 1078:
            self.graph_index = len(read_record) - 1078   # stop index from going over max
        ...
        for num, column in enumerate(display_vals.T):   # for every column in val array, (transpose to read rows)
            if num != 0:   # skip first column - it is x value
                ...
                if antial is True:
                    pygame.draw.aalines(self.surface, color, False, val_out, 2)   # draw graphed lines
                else:
                    pygame.draw.lines(self.surface, color, False, val_out, 2)   # draw graphed lines
        # time scale
